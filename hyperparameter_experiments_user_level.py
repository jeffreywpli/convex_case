...
def run_experiment(dp_notion='user_level', max_grad_norm=10.0, noise_multiplier=0.51,
	...
	seeds=[0], transductive=True, sgd=False):
	...
	print(learning_rate)
	print(train_shots)
	result_dir = './results/miniimagenet/hyper_search/5_shot_5_way/user_level/meta_batches_{0:.1f}_meta_iters_{1:.1f}_grad_{2:.5f}_noise_multiplier_{3:.4f}_meta_step_{4:.4f}_learning_rate_{5:.4f}_train_shots{6:.2f}'.format(
		meta_batch, meta_iters, max_grad_norm, noise_multiplier, meta_step, learning_rate, train_shots)
	...
	if max_acc > 0.09:
		for i, seed in enumerate(seeds):
			...
			commands = ['python3', 'run_miniimagenet_user_level.py']
			commands.extend(
...
if search == "initial_tuning":
	...
	# Privacy Params
	max_grad_norm = [1.3, 1.0, 0.7, 0.4, 0.1]
...
experiments = list(itertools.product(max_grad_norm, meta_batch, meta_step, learning_rate, train_shots))
np.random.seed(234)
perm = np.random.choice(len(experiments), choices, replace=False)

for i in perm:
	max_grad_norm, meta_batch, meta_step, learning_rate, train_shots = experiments[i]

	#meta_iters = int(100000 / meta_batch)
	#eval_interval = meta_iters // 10
